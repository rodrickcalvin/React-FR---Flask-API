# The Bookshelf API
> A Flask API for a bookshelf app from the Udacity 
This project s a virtual bookshelf for Udacity students. Users are able to add their books to the bookshelf, give them a rating, update the rating and search through their book lists. As a part of the Fullstack Nanodegree, it serves as a practice module for lessons from Course 2: API Development and Documentation. By completing this project, I learnt and applied the skills in structuring and implementing well formatted API endpoints that leverage knowledge of HTTP and API development best practices. 



#### Pre-requisites
* Developers using this project should already have Python3, pip and node(LTS) installed on their local machines.
...
In your terminal, navigate to the */React-FR---Flask-API/Bookshelf-API/backend/* directory, and run the following:
```bash
cd React-FR---Flask-API/Bookshelf-API/backend
...
### Step 2: Start the backend server & add more logic to the API
Navigate to the `/backend/flaskr/__init__.py` file, and add more routes and logic to get the backend of your app better and running.
